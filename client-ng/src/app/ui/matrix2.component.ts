...
export class Matrix2Component implements AfterViewInit, OnChanges, OnDestroy {
    ...
    set actorPos(value: Position) {
        // prevent unneeded rendering
        if (!this.actorPos_ || this.actorPos_.x !== value.x || this.actorPos_.y !== value.y) {
            this.actorPos_ = value;
            this.drawPos();
        }
    }
    ...
    private drawPos() {
        ...
        ctx.beginPath();
        if (this.actorPos_ == undefined) {
            return;
        }

        let x = this.normalizeAndConvertX(this.actorPos_.x) % this.width;
